...
app.post("/buy-server4-number", async (req, res) => {
  ...
  const { data: settings } = await client.from("server4_settings").select("api_key, profit_margin_percent").limit(1).maybeSingle();
  ...
  for (let i = 0; i < qty; i++) {
    const { data: freshProfile } = await client.from("profiles").select("wallet_balance, referral_balance, wallet_hold").eq("id", profile.id).single();
    ...
    await client.rpc("ensure_wallet_funds_from_referral", { p_user_id: profile.id, p_needed: unitPrice });
    const { data: held } = await client.rpc("hold_wallet", { p_user_id: profile.id, p_amount: unitPrice });
    if (!held) {
    ...
    await client.from("server4_orders").insert({
      user_id: profile.id,
